...
export default function ReportsPage() {
  ...
  const handleDownloadPDF = async () => {
    try {
      ...
      const accentBlue = [30, 58, 138];       // Blue 900
      const brandBlue = [37, 99, 235];        // Blue 600
      const primaryBlue = brandBlue;
      const secondaryPurple = [109, 40, 217]; // Purple 700
      const successEmerald = [5, 150, 105];   // Emerald 600
      const dangerRed = [220, 38, 38];        // Red 600
      const warningAmber = [217, 119, 6];     // Amber 600
      const darkSlate = [30, 41, 59];         // Slate 800
      const slateText = [71, 85, 105];        // Slate 600
      const mutedText = [148, 163, 184];      // Slate 400
      const lightBg = [248, 250, 252];        // Slate 50
      const borderGray = [226, 232, 240];     // Slate 200

      // Unique Cryptographic Audit Hash Reference
      const pdfRefHash = `REF-${Math.floor(100000 + Math.random() * 900000)}`;

      // Safe currency formatting helper to prevent character code glitches in jsPDF Helvetica
      ...
      const drawHeaderBanner = (pageTitle: string) => {
        // Dual accent strip at top
        doc.setFillColor(primaryNavy[0], primaryNavy[1], primaryNavy[2]);
        doc.rect(0, 0, 130, 3.5, "F");
        doc.setFillColor(brandBlue[0], brandBlue[1], brandBlue[2]);
        doc.rect(130, 0, 80, 3.5, "F");

        doc.setTextColor(primaryNavy[0], primaryNavy[1], primaryNavy[2]);
        doc.setFont("helvetica", "bold");
        doc.setFontSize(13);
        doc.text("MY ACCOUNTING", 20, 15);

        doc.setFontSize(7.5);
        doc.setFont("helvetica", "bold");
        doc.setTextColor(brandBlue[0], brandBlue[1], brandBlue[2]);
        doc.text("EXECUTIVE FINANCIAL REPORTING ENGINE", 20, 18.5);

        doc.setTextColor(slateText[0], slateText[1], slateText[2]);
        doc.setFont("helvetica", "bold");
        doc.setFontSize(8);
        doc.text(pageTitle.toUpperCase(), 190, 16.5, { align: "right" });
      ...
      const addFooter = (pNum: number, total: number) => {
        ...
        doc.text(`CONFIDENTIAL AUDIT • Generated by My Accounting System • Hash: #${pdfRefHash}`, 20, 287);
        doc.setFont("helvetica", "bold");
